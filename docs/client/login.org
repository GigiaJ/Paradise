...
(defn start-sync! [client]
  (p/let [sync-service (-> (.syncService client) (.withOfflineMode) (.finish))
    ...
    (let [listener    (create-room-update-listener)
          setup-done? (atom false)]
      (.start sync-service)
      (log/debug "Sync started. Waiting for 'Loaded' state...")
      (let [loading-result
            (.loadingState room-list
                           #js {:onUpdate
                                (fn [state]
                                  (let [tag (.-tag state)]
                                    (when (and (= tag "Loaded") (not @setup-done?))
                                      (reset! setup-done? true)
                                      (setup-room-list-adapter! room-list listener))))})]
        (swap! sdk-world assoc-in [:vms :room-list :state-stream] (.-stateStream loading-result))))))
#+end_src


We're moving on with making RoomViewModel work in its entirety, but we still
need to stub a few methods in like TimelineViewModel is a lengthy one to implement,
so it makes more sense to stub and move on for now.
#+begin_src clojurescript :tangle ../../src/client/login.cljs
...
Currently, however, it is blocked by implementing i18n as the components are picky
