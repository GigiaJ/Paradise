...
(ns container.search
  (:require
   ...
   [utils.svg :as icons]
   [container.reusable :refer [make-timeline-item-shim]]
...
Now for the components themselves. Each one is just a wrapped event tile with
a convenient click-able jump button.
It may be reasonable to reuse this for Pins.

#+begin_src clojurescript :tangle ../../src/container/search.cljs
(defn search-result-item [room-id result]
  [:div.search-hit-card
   [event-tile result]
   [:button.jump-btn
    {:on-click #(re-frame/dispatch [:room/pretty-jump room-id (:id result)])}
    "Jump to Message"]])
#+end_src


Now for the main component itself. We want to provide some nice visual
impact to the user. At some point down the road we'd like to provide
       ...
       [:div.search-results-area
        (cond
          ...
          [:div.search-list
           (for [res results]
             ^{:key (:id res)}
             [search-result-item active-room res])]
